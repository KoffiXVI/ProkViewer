...
from database_maintenance_functions import Database_Ops_Handler #table_operation
from database_constants import *

class Database_Manager(Database_Ops_Handler):
    def __init__(self, db_name:str=PROK_DB_PATH, db_path:str=DB_STORE_PATH):
        super().__init__(db_name)

        self.db_path = db_path

    def create_prok_database(self, force:bool=False):
    ...
    def create_temp_database(self):
        ...
        try:
            for command in (prokaryote_command, nodes_command, names_command, cog_func_command, cog_names_command):
                self.table_operation(command, (), many=False, terminate=False)
        except Exception as e:
            return (False, self.error_handler(e))
    ...
    def load_cog_names(self, sourcefile:bytes|str, target_table:str=TEMP_COG_NAMES_TABLE):
    
        values = self.process_text_filedata(sourcefile, "\n","\t", interest=[0, 1, 2])
    ...
    def load_cog_func(self, sourcefile:bytes|str, target_table:str=TEMP_COG_FUNC_TABLE):
    ...
    def extract_COG_base(self, db_link:str = COG_DATABASE_LINK, db_local:str|None = None, 
                        func_link:str = COG_FUNCTIONNAL_LINK, 
                        func_local:str|None = None, fam_link:str = COG_FAMILY_LINK, fam_local:str|None = None):
        
        process = [(db_link, db_local, self.load_disk_cog_database), 
                (func_link, func_local, self.load_cog_func), 
                (fam_link, fam_local, self.load_cog_names)]
    ...
    # TAXONOMY OPERATIONS
    def load_nodes(self, sourcefile:bytes|str, target_table:str=TEMP_NODES_TABLE):
        values = self.process_text_filedata(sourcefile, "\t|\n","\t|\t", interest=[0, 1])
    ...
    def load_names(self, sourcefile:bytes|str, target_table:str=TEMP_NAMES_TABLE):
        if not isinstance(sourcefile, (str, bytes)):
            raise TypeError(f"Expected bytes or str for the source file, got {type(sourcefile).__name__} instead")
    ...
    def extract_tax_dump(self, source:str = TAXDMP_LINK, tax_local:str|None=None, nodes_filename:str=NODES_FILE, names_filename:str=NAMES_FILE):
        
        target_files = [(nodes_filename, self.load_nodes),(names_filename,self.load_names)]
    ...
    def extract_prok_data(self, source:str = PROK_LINK, local_source:str|None=None, db_name:str = PROK_DB_PATH, target_table:str=TEMP_PROK_TABLE):
    ...
    def create_working_database(self, prok_source:str, prok_local:str|None, tax_source:str, tax_local:str|None,
        ...
        self.create_prok_database(force=force)

        if not os.path.exists(GENOME_FOLDER):
            os.makedirs(GENOME_FOLDER)

        self.update_database(prok_source, prok_local, tax_source, tax_local,
                            cog_db_link, cog_db_local, fam_link, fam_local, 
                            cog_func_link, cog_func_local)
        
    def update_database(self, prok_source:str, prok_local:str|None, tax_source:str, tax_local:str|None,
                        cog_db_link:str, cog_db_local:str|None, fam_link:str, fam_local:str|None,
                        cog_func_link:str, cog_func_local:str|None, connector:sqlite3.Connection|None=None):
        
        self.create_temp_database()
        processes = (
            ("extracting taxonomy data", self.extract_tax_dump, (tax_source, tax_local)), 
            ("extracting genomic data", self.extract_prok_data, (prok_source, prok_local)), 
            ("extracting COG data", self.extract_COG_base, (cog_db_link, cog_db_local, cog_func_link, cog_func_local,
                                                    fam_link, fam_local)),
            ("updating database", self.fuse_temp_database, ())
        )
        for process_message, process, vars in processes:
            print(process_message)
            valid, result = process(*vars) if len(vars) else process()
            if not valid:
